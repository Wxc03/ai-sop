...
`src/SwSopAddin.Tests/` — MSTest 3.6.4, hand-rolled mocks in `Mocks/MockServices.cs` (no Moq dependency). Coverage today: `SopWorkflow` early-exit + `SopResult` defaults; `ExplodeService.ShouldSkip` / `GetComponentBaseName`; `AiAdvisorOptions.EffectiveApiKey` / `ApiKeySource`; `AiExplodeAdvisor.ComputeChangeSet` / `ApplyRebuild` (uses a `RecordingEditor : IExplodeStepEditor` fake); `RunStep` dispatch + out-of-range; `ParseM3Response` JSON tolerance; `ExplodeLayoutPlanner.Plan` role classification/coaxial grouping/direction math (`ExplodeLayoutPlannerTests.cs`); `ExplodeService.ApplyPlan` against a `RecordingExplodeApplier` fake (`ExplodeServiceSmartHybridTests.cs`); `SopLayoutPlanner.Compute` iso-centering geometry (`SopLayoutPlannerTests.cs`); a real-SW-API smoke test for the 4 layout-writability calls (`SwLayoutApiSmokeTests.cs`). **Full 7-step COM pipeline tests now exist** (`SopWorkflowTests.cs`) — the W9 `IDocumentValidator` refactor (see below) made Step 0 mockable, so `MockDocumentValidator` drives WrongType / null-asm / validator-throws / full-7-step cases. ~130+ test methods, all passing.
...
To run a single test class or method from the CLI: `vstest.console.exe src\SwSopAddin.Tests\bin\Debug\SwSopAddin.Tests.dll /Tests:ExplodeLayoutPlannerTests` (class name) or `/Tests:ExplodeLayoutPlannerTests.Plan_EmptyInput_ReturnsEmptyResult` (fully-qualified method). In VS Test Explorer, right-click the individual test → Run.
...
| `SwSopAddin.Services` | One file per "M" step: `ExplodeService` (M2), `ViewService` (M4), `BalloonService` (M5), `BomService` (M6), `DrawingService` (M2.5). Plus `AiExplodeAdvisor` (M7) — calls M3 vision API in a 3-round loop, modifies explode steps. `ExplodeService.Create` is a dispatcher: `ConfigStore.ExplodeOptions.Style` (default `SmartHybrid`) picks between the original `LegacyCreate` (3-fallback: `IAddExplodeStep` per component → `AutoExplode` → `TranslateComponent`) and `SmartHybridCreate` (`ExplodeLayoutPlanner.Plan` computes per-component direction/distance from geometry, `SwDirectionalExplodeApplier` — an `IExplodeApplier` — applies each placement via `IAddExplodeStep` after selecting a matching reference plane). Any exception or 0 successful placements in SmartHybrid falls back to `LegacyCreate`. Each service has an `I*Service` interface for testability. | Adapter, SW interop |
| `SwSopAddin.Layout` | Two coexisting mechanisms. **W5 avoidance** (`LayoutService.ApplyLayout`): bounding box collection → collision detection → avoidance → out-of-bounds scaling → write-back. Pipeline: `BoundingBoxCollector` → `CollisionDetector` → `AvoidanceResolver` → `OutOfBoundsScaler` → (legacy `CenterMovableElements`, slated for removal) → `LayoutApplier`. **Iso auto-placement** (`LayoutService.ApplyIsoPlacement`, newer): `SopLayoutPlanner.Compute` computes a target rect + scale factor purely from sheet size and the iso view's current outline; the caller writes `View.ScaleRatio`/`View.Position` back. `LayoutService.GetSheetBounds` prefers real `Sheet.GetSize` and falls back to a `PaperSize` string table (A3 default) only if that fails. | Services types (View, BomTableAnnotation) |
...
2. **Step 1** M2 explode: `ExplodeService.Create` dispatches on `config.Explode.Style` (default `SmartHybrid`). **SmartHybrid** (`SmartHybridCreate`): geometry-driven placement via `ExplodeLayoutPlanner.Plan` + `SwDirectionalExplodeApplier.ApplyPlacement` (per component, picks whichever of Right/Front/Top Plane has normal closest to the target direction, selects component+plane as a 2-item selection set, then calls `cfg.IAddExplodeStep`). **Both SmartHybrid and Legacy must call `asm.CreateExplodedView()` + `ShowExploded2(true, name)` *before* the per-component loop, not after** — `IAddExplodeStep`/`AddExplodeStep2` return `Error=2 (NoExplodeView)`/null 100% of the time on a configuration with no pre-existing explode view (confirmed via real-machine diagnostic with `AddExplodeStep2`'s `Error` out-param, fixed 2026-07-12). **Legacy** (`LegacyCreate`) keeps its original 3-fallback path — `IAddExplodeStep` returns null on multi-component assemblies (BenchVice/PressTool pattern instances), so it falls back to (a) `asm.AutoExplode()` SW heuristic then (b) `asm.TranslateComponent()` to manufacture real steps. **Light-weight resolution** (`ResolveAllLightWeightComponents(false)`) is called first in both paths because light-weight components break `IAddExplodeStep`.
...
8. **Step 6** W5 layout: `LayoutService.ApplyLayout` — F14 collision avoidance + F15 out-of-bounds scaling, plus `ApplyIsoPlacement` for iso view auto-centering/scaling (see Layout row above). F16 pagination is not implemented (pending task); a further refactor to remove the legacy `CenterMovableElements` in favor of `SopLayoutPlanner`-driven placement is also pending (see task list / `w14-layout-step6-never-ran-and-usablew-bug` memory).
...
- **`IAddExplodeStep`/`AddExplodeStep2` fail with `Error=2 (NoExplodeView)`/null on a configuration that has no pre-existing explode view.** Both `ExplodeService.LegacyCreate` and `SmartHybridCreate` therefore call `CreateAndActivateExplodedView` (which does `asm.CreateExplodedView()` + `ShowExploded2(true, name)`) *before* the per-component step-adding loop — creating the view only in a "finalize" step *after* the loop (the original bug) causes every `IAddExplodeStep` call to fail.
- **`SwDirectionalExplodeApplier` needs a 3-segment `SelectByID2` name for a sub-component's reference plane**: `"<planeName>@<componentName>@<topLevelAssemblyTitle>"` — the 2-segment form (`"<planeName>@<componentName>"`) fails 100% of the time on real hardware. `_asmTitle` must have its `.sldasm` extension stripped first (`GetTitle()` includes it for assemblies, unlike parts).
...
`Explode.Style` (`"SmartHybrid"` default, `"Legacy"` fallback) picks the M2 explode engine — set to `"Legacy"` to instantly roll back to the pre-Part-A explode behavior without a code change if SmartHybrid misbehaves on a new assembly. `Explode.FastenerNameKeywords` is a `string[]` used by `ExplodeLayoutPlanner.ClassifyRole` — extend it with assembly-specific part-naming conventions (e.g. `"导柱"`, `"导套"`, `"pillar"`, `"bush"`) so physically-coaxial fastener pairs get classified into the same `Role` and stack coherently instead of diverging.
...
4. The relevant `*Service.cs` for whatever step you're touching (e.g. `ExplodeService.cs` for M2, `ViewService.cs` for M4). For M2 SmartHybrid specifically, also read `ExplodeLayoutPlanner.cs` (pure geometry: role classification, coaxial grouping, direction/distance) and `SwDirectionalExplodeApplier.cs` (the `IExplodeApplier` that turns a planned direction into a real `IAddExplodeStep` call).
...
7. `C:\Users\mozhi\.claude\projects\D--Source-SwSopAddin\memory\MEMORY.md` indexes all cross-session *why* rationale — at minimum read `w3-m4-m6-root-cause.md` and `w6-fix-m2-m4-m5-m6-wiring.md` before touching M2/M4/M5/M6, and `w15-explode-phase3b-plane-selection-fix.md` before touching the SmartHybrid explode path.
...
|---|---|---|
| 1 explode step (expected 10+) | `Phase 1` `processed/failed/totalComps` (Legacy) or `SmartHybridCreate: ApplyPlan done processed=X failed=Y` (SmartHybrid) | Default `SkipNamePrefixes` is `[]`; if still 1 step, check asm actual component count (pattern / light-weight instances may need `ResolveAllLightWeightComponents`) |
| All components fail with `Error=2(NoExplodeView)` / `IAddExplodeStep 返 null` | `CreateAndActivateExplodedView` log lines | Should already be fixed — confirm `CreateAndActivateExplodedView` runs (and its `view name='...'` log line appears) *before* the `ApplyPlan`/component loop, not after |
...
- SmartHybrid `NoExplodeView` fix (2026-07-12) is real-machine-confirmed on Piston (13/13 components, 0 failures) but **not yet retested on PressTool/BenchVice** — those were the assemblies that originally exercised the coaxial-pair (导柱/导套) classification fix, so a regression check on them is still pending.
- W5 layout is mid-refactor: the legacy `CenterMovableElements` step inside `LayoutService.ApplyLayout` and the newer `SopLayoutPlanner`-driven `ApplyIsoPlacement` currently coexist; removing the former in favor of the latter is planned but not done.
